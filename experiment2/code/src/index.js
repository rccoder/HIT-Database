...
let PNO, PNAME, ENAME, DNAME, SALARY, ESSN, PNO1, PNO2, N, HOURS;
...
let handle_query = (q, p) => {

    ({PNO, PNAME, ENAME, DNAME, SALARY, ESSN, PNO1, PNO2, N, HOURS} = p);
...
let app = (argv) => {
    ...
    const queryItem = handle_query(q, p);
    
    console.log(`SQL: ${queryItem}`);
    connection.query(queryItem, (err, rows, fields) => {
        if(err) {
